...
export default function AdminBlogPage() {
  ...
  const fetchBlogs = async () => {
    try {
      const res = await fetch("/api/v1/blog?page=1&page_size=100", { headers });
      if (res.ok) {
  ...
  return (
    <div>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--text-secondary)", marginTop: "0.25rem" }}>Manage articles, news, and insights published to the platform</p>
      </div>
