...
export default function InstructorDashboard() {
  ...
  return (
    <div style={{ minHeight: "80vh" }}>
      {/* Banner */}
      <div style={{ background: "var(--card-bg)", border: "1px solid var(--border-color)", borderRadius: "var(--radius-md)", padding: "2rem", marginBottom: "2.5rem" }}>
        ...
        <h1 style={{ fontSize: "2rem", fontWeight: 800, marginTop: "0.25rem", color: "var(--text-primary)" }}>Welcome, {user?.full_name}</h1>
        <p style={{ color: "var(--text-secondary)" }}>Manage your course curricula, syllabus lectures, and view student progress stats.</p>
      </div>

      {/* Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "3rem" }} className="responsive-grid-split">
        {/* Stats overview */}
        <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
          <div style={{ background: "var(--card-bg)", border: "1px solid var(--border-color)", borderRadius: "12px", padding: "1.75rem" }}>
          ...
          <div style={{ background: "var(--card-bg)", border: "1px solid var(--border-color)", borderRadius: "12px", padding: "1.75rem" }}>
            <h4 style={{ fontWeight: 700, fontSize: "1.1rem", marginBottom: "1rem", color: "var(--text-primary)" }}>Academic Guidelines</h4>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            {data.courses.length === 0 ? (
              <div style={{ background: "var(--card-bg)", border: "1px solid var(--border-color)", borderRadius: "12px", padding: "4rem 2rem", textAlign: "center" }}>
                <p style={{ color: "var(--text-muted)", fontSize: "0.95rem" }}>No active courses assigned to you.</p>
              </div>
            ) : (
              data.courses.map((course: any) => (
                <div key={course.course_id} style={{ background: "var(--card-bg)", border: "1px solid var(--border-color)", borderRadius: "var(--radius-md)", padding: "1.5rem", display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: "1rem" }}>
                    <div>
                      ...
                      <h4 style={{ fontWeight: 700, fontSize: "1.15rem", marginTop: "0.5rem", color: "var(--text-primary)" }}>{course.title}</h4>
